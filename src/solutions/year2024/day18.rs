...
impl Memory {
    ...
    pub fn find_shortest_path_to_exit(
        &mut self,
        exit: BytePosition,
    ) -> Result<(i32, Vec<BytePosition>)> {
        let mut bytes: VecDeque<Props> = VecDeque::new();
        bytes.push_back(((0, 0), 0));

        let mut parents: Vec<Vec<Option<BytePosition>>> =
            vec![vec![None; self.space.cols as usize]; self.space.rows as usize];
...
pub fn solve_part1() -> Result<i32> {
    ...
    let _ = memory.mark_corrupted_bytes(
        data.iter()
            .take(1024)
            .cloned()
            .collect::<Vec<BytePosition>>(),
    );
